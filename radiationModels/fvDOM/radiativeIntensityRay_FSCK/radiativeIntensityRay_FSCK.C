...
#include "radiativeIntensityRay_FSCK.H"
#include "fvm.H"
...
const Foam::word
Foam::radiation::radiativeIntensityRay_FSCK::intensityPrefix("ILambda");


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::radiation::radiativeIntensityRay_FSCK::radiativeIntensityRay_FSCK
(
    ...
    myRayId_(rayId)
{
    ...
    if (mesh_.nSolutionD() == 2)
    {
        // Omega for 2D
        //omega_ = deltaPhi;
        omega_ = 2.0*deltaPhi; // possible fix for total_omega = 4*pi
        ...
        // dAve for 2D (Fixes 2D)
        dAve_ = vector
        (
            pi*sinPhi*Foam::sin(0.5*deltaPhi),
            pi*cosPhi*Foam::sin(0.5*deltaPhi),
            0
        );
...
Foam::scalar Foam::radiation::radiativeIntensityRay_FSCK::correct_FSCK
(
    const volScalarField& k,
    const volScalarField& aScale
)
{
    ...
    //label j = absorptionEmission_.bandGet();
    ...
    (
        ...
        mesh_,
        dimensionedScalar(dimless/dimLength,1e2*kfsck[j])
    );*/

    //Info<<"The k value is "<<kfsck[j]<<endl;
    ...
    forAll(ILambda_, lambdaI)
    {
        ...
        ==
           ...
           *(
                k*aScale*blackBody_.bLambda(lambdaI)
...
void Foam::radiation::radiativeIntensityRay_FSCK::addIntensity()
{
