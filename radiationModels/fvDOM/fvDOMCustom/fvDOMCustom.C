...
namespace Foam
{
    namespace radiation
    {
        defineTypeNameAndDebug(fvDOMCustom, 0);
        addToRadiationRunTimeSelectionTables(fvDOMCustom);
    }
}
...
void Foam::radiation::fvDOMCustom::rotateInitialRays(const vector& sunDir)
{
...
void Foam::radiation::fvDOMCustom:: alignClosestRayToSun(const vector& sunDir)
{
    label SunRayId(-1);
...
void Foam::radiation::fvDOMCustom::updateRaysDir()
{
...
void Foam::radiation::fvDOMCustom::initialise()
{
    ...
    if (mesh_.nSolutionD() == 3)
    {
        ...
        for (label n = 1; n <= nTheta_; n++)
        {
            for (label m = 1; m <= 4*nPhi_; m++)
            {
                ...
                (
                    i,
                    new radiativeIntensityRayCustom
                    (
                        *this,
                        mesh_,
                        phii,
    ...
    Info<< "fvDOMCustom : Allocated " << IRay_.size()
    ...
    if (useSolarLoad_)
    {
        ...
        {
            FatalErrorInFunction
                << "External beam with fvDOMCustom can not be used "
        ...
        {
            FatalErrorInFunction
                << "Requested solar radiation with fvDOMCustom. Using "
...
Foam::radiation::fvDOMCustom::fvDOMCustom(const volScalarField& T)
:
...
Foam::radiation::fvDOMCustom::fvDOMCustom
(
...
bool Foam::radiation::fvDOMCustom::read()
{
...
void Foam::radiation::fvDOMCustom::calculate()
{
    ...
    delqr_= a_*(4.0*physicoChemical::sigma*pow4(T_) - G_);
}


Foam::tmp<Foam::volScalarField> Foam::radiation::fvDOMCustom::Rp() const
{
...
Foam::radiation::fvDOMCustom::Ru() const
{
...
void Foam::radiation::fvDOMCustom::updateBlackBodyEmission()
{
    for (label j=0; j < nLambda_; j++)
    {
...
void Foam::radiation::fvDOMCustom::updateG()
{
...
void Foam::radiation::fvDOMCustom::setRayIdLambdaId
(
    const word& name,
    label& rayId,
    label& lambdaId
...
const Foam::solarCalculator& Foam::radiation::fvDOMCustom::solarCalc() const
{
