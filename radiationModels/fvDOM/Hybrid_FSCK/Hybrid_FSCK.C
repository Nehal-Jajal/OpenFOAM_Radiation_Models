...
namespace Foam
{
    namespace radiation
    {
        defineTypeNameAndDebug(Hybrid_FSCK, 0);
        addToRadiationRunTimeSelectionTables(Hybrid_FSCK);
    }
}
...
void Foam::radiation::Hybrid_FSCK::rotateInitialRays(const vector& sunDir)
{
...
void Foam::radiation::Hybrid_FSCK:: alignClosestRayToSun(const vector& sunDir)
{
    label SunRayId(-1);
...
void Foam::radiation::Hybrid_FSCK::updateRaysDir()
{
...
void Foam::radiation::Hybrid_FSCK::initialise()
{
...
Foam::radiation::Hybrid_FSCK::Hybrid_FSCK(const volScalarField& T)
:
...
Foam::radiation::Hybrid_FSCK::Hybrid_FSCK
(
...
bool Foam::radiation::Hybrid_FSCK::read()
{
...
void Foam::radiation::Hybrid_FSCK::calculate()
{
    ...
    Info<<"\n Running FSCK version of Hybrid (hardcoded) \n"<<endl;
    ...
    for (label j=0; j < 8; j++)
    {
        ...
        do
        {
            ...
            forAll(IRay_, rayI)
            {
                if (!rayIdConv[rayI])
                {
                    scalar maxBandResidual = IRay_[rayI].correct_FSCK();
        ...
        delqr_= delqr_ + wq[j]*dimensionedScalar(dimless/dimLength,1e2*kfsck[j])*(4.0*physicoChemical::sigma*pow4(T_) - G_);
    }
...
Foam::tmp<Foam::volScalarField> Foam::radiation::Hybrid_FSCK::Rp() const
{
...
Foam::radiation::Hybrid_FSCK::Ru() const
{
...
void Foam::radiation::Hybrid_FSCK::updateBlackBodyEmission()
{
    for (label j=0; j < nLambda_; j++)
    {
...
void Foam::radiation::Hybrid_FSCK::updateG()
{
...
void Foam::radiation::Hybrid_FSCK::setRayIdLambdaId
(
    const word& name,
    label& rayId,
    label& lambdaId
...
const Foam::solarCalculator& Foam::radiation::Hybrid_FSCK::solarCalc() const
{
