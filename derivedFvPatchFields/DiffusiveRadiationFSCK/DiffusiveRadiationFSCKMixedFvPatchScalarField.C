...
#include "DiffusiveRadiationFSCKMixedFvPatchScalarField.H"
...
Foam::radiation::DiffusiveRadiationFSCKMixedFvPatchScalarField::
DiffusiveRadiationFSCKMixedFvPatchScalarField
(
    const fvPatch& p,
...
DiffusiveRadiationFSCKMixedFvPatchScalarField
(
    const DiffusiveRadiationFSCKMixedFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::radiation::DiffusiveRadiationFSCKMixedFvPatchScalarField::
updateCoeffs()
{
    if (this->updated())
    {
    ...
    // This works for FSCK nongrey case (FIX THIS)
    if (dom.nLambda() != 1)
    {
    ...
    forAll(Iw, faceI)
    {
        if ((-n[faceI] & myRayId) > 0.0)
        {
            ...
            //Info << "The boundary face id is " << bFaceIdx << " and scaling a is " << aBF[bFaceIdx] << endl;
...
void Foam::radiation::DiffusiveRadiationFSCKMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
