...
#include "greyDiffusiveRadiationCustomMixedFvPatchScalarField.H"
...
Foam::radiation::greyDiffusiveRadiationCustomMixedFvPatchScalarField::
greyDiffusiveRadiationCustomMixedFvPatchScalarField
(
    const fvPatch& p,
...
greyDiffusiveRadiationCustomMixedFvPatchScalarField
(
    const greyDiffusiveRadiationCustomMixedFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::radiation::greyDiffusiveRadiationCustomMixedFvPatchScalarField::
updateCoeffs()
{
    if (this->updated())
    {
    ...
    const fvDOMCustom& dom = db().lookupObject<fvDOMCustom>("radiationProperties");
    // ModificTION FOR fsck
    //const fvDOM_FSCK& dom = db().lookupObject<fvDOM_FSCK>("radiationProperties");
    //const radiationModel& radmodel = db().lookupObject<radiationModel>("radiationProperties");
    //const label bandI = radmodel.absorptionEmission().bandGet();
    // Wall @ 1500K
    //scalar af[8] = {0.831306,0.882217,0.920929,0.756753,0.686080,0.635293,0.665711,0.618451};
    ...
    radiativeIntensityRayCustom& ray =
        const_cast<radiativeIntensityRayCustom&>(dom.IRay(rayId));
    // Modification for fsck
    //radiativeIntensityRay_FSCK& ray =
    //   const_cast<radiativeIntensityRay_FSCK&>(dom.IRay(rayId));
...
void Foam::radiation::greyDiffusiveRadiationCustomMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
